...
/*   Updated: 2024/08/02 22:32:15 by msennane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						flood_fill(t_game_map *map, int col, int row,
								char **visited);
int							has_valid_path(t_game_state *params);
char						**allocate_visited_array(int rows, int cols);
void						initialize_visited_array(char **visited, int rows,
								int cols);
...
void						load_map_from_queue(t_map_parse_queue *queue,
								t_game_map *map);
...
int							count_map_characters(const t_game_map *map,
								char target_char);
...
int							process_arguments_and_map(int argc, char **argv,
								t_game_state *state);
int							validate_command_line_arguments(int argc,
								char **argv, t_game_state *state);
